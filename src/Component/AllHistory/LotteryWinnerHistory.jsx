...
const tableHead = [
  ...
  { id: 5, name: "Username", width: 140 },
  { id: 3, name: "Ticket Id", width: 120 },
  { id: 4, name: "Prize Type", width: 120 },
];
const LotteryWinnerHistory = ({ isLoading, setIsLoading }) => {
  ...
  const handleData = () => {
    axios
      .get(
        `${process.env.REACT_APP_MAIN_URL}${ENDPOINT?.ticket?.drawLotteryWinners}${id?.id}`,
        {
          headers: {
  ...
  useEffect(() => {
    ...
    function filter() {
      if (searchState !== "") {
        tableDatas.map((dat) => {
          if (
            dat.username.toLowerCase().search(searchState.toLowerCase()) !== -1
          ) {
            arr.push(dat);
          }
          return true;
  ...
  return (
    <Fragment>
      <PrimaryLayout pageTitle="All Draw Ticket">
...
export default LotteryWinnerHistory;
