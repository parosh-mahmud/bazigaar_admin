...
import CreateSlider from "../../Component/Slider/CreateSlider";
import Modal from "../../Component/Modals/Modal";
import axios from "axios";
import { ENDPOINT } from "../../App/config/endpoint";
import { toast } from "react-hot-toast";
import { UnAuth } from "../Auth/UnAuth";
// import ToggleButton from "../../Component/Shared/ToggleButton";
...
const tableHead = [
  ...
  { id: 4, name: "Created Date", width: 120 },
  { id: 5, name: "Status", width: 40 },
  { id: 6, name: "Action", width: 40 },
];

const OnboardingSlider = ({ isLoading, setIsLoading }) => {
  ...
  const [modalContent, setModalContent] = useState(
    <CreateSlider setModalOpen={setModalOpen} />
  );
  ...
  const getData = async () => {
    ...
    try {
      const response = await axios.get(
        `${process.env.REACT_APP_MAIN_URL}${ENDPOINT.slider.getSlider}`,
        {
          headers: {
  ...
  useEffect(() => {
    ...
    function filter() {
      if (searchState !== "") {
        tableDatas.map((dat) => {
          if (
            dat.title.toLowerCase().search(searchState.toLowerCase()) !== -1
          ) {
            arr.push(dat);
          }
          return true;
  ...
  const handleModal = (e) => {
    ...
    setModalOpen(true);
    setModalContent(
      <CreateSlider setModalOpen={setModalOpen} getData={getData} />
    );
  };

  const handleDelete = (e, id, DltTitle) => {
    e.preventDefault();
    const endpoint = `${process.env.REACT_APP_MAIN_URL}${ENDPOINT?.slider?.deleteSlider}${id}`;
    setModalOpen(true);
    setModalContent(
      <DeleteCompo
  ...
  return (
    <PrimaryLayout pageTitle="Manage Slider">
        ...
        <div className="flex w-full flex-col items-center gap-4 md:w-[unset] md:flex-row">
          <div className="flex w-full items-center">
            <input
              ...
              placeholder="Search slider here..."
                ...
                return (
                  ...
                  >
                    ...
                    <td className="px-2">
                      <div className="h-10 w-10">
                        <img
                          ...
                          src={`${process.env.REACT_APP_MAIN_URL}${tbody?.sliderImage}`}
                          alt={tbody?.title}
                        />
                      </div>
                    </td>
                    ...
                    <td className="cursor-pointer px-2">
                      {tbody?.active === true || tbody?.active === "True" ? (
                        <p className="low w-fit rounded-full bg-[#63ad6f24] px-3 py-2 font-inter text-[14px] font-medium capitalize text-[#63AD6F]">
                          Active
                        </p>
                      ) : (
                        <p className="low w-fit rounded-full bg-[#ca755924] px-3 py-2 font-inter text-[14px] font-medium capitalize text-[#c14029]">
                          Inactive
                        </p>
                      )}

                      {/* <ToggleButton apiStatus={tbody?.active} /> */}
                    </td>
                    <td className="px-2">
                      <div className="flex items-center gap-3">
                        <svg
                          onClick={(e) =>
                            handleEdit(e, tbody?.id, tbody?.title)
                          }
                        ...
                        <svg
                          onClick={(e) =>
                            handleDelete(e, tbody?.id, tbody?.title)
                          }
                          className="cursor-pointer"
                          xmlns="http://www.w3.org/2000/svg"
                          width="12"
