...
import CreateBanking from "../../Component/Wallet/CreateBanking";
import { WalletID } from "../../Assets/locales/WalletID";
const tableHead = [
  { id: 1, name: "SL", width: 10 },
  { id: 2, name: "Image", width: 40 },
  { id: 7, name: "Bank Name", width: 150 },
  { id: 3, name: "Branch Name", width: 150 },
  { id: 4, name: "Account Holder Name", width: 200 },
  { id: 8, name: "Account Number", width: 200 },
  // { id: 5, name: "Status", width: 40 },
  { id: 6, name: "Action", width: 40 },
];

const Banking = ({ isLoading, setIsLoading }) => {
  ...
  const [modalContent, setModalContent] = useState(
    <CreateBanking setModalOpen={setModalOpen} />
  );
  ...
  const getData = async () => {
    ...
    try {
      const response = await axios.get(
        `${process.env.REACT_APP_MAIN_URL}${ENDPOINT.wallet.getBank}${
          WalletID().wallet_id
        }/`,
        {
          headers: {
        ...
        tableDatas.map((dat) => {
          if (
            ...
            dat.accountHolderName
              .toLowerCase()
              .search(searchState.toLowerCase()) !== -1 ||
            dat.branchName.toLowerCase().search(searchState.toLowerCase()) !==
              -1
          ) {
            arr.push(dat);
          }
  ...
  const handleModal = (e) => {
    ...
    setModalOpen(true);
    setModalContent(
      <CreateBanking setModalOpen={setModalOpen} getData={getData} />
    );
  };

  const handleDelete = (e, id, DltTitle) => {
    e.preventDefault();
    const endpoint = `${process.env.REACT_APP_MAIN_URL}${ENDPOINT?.wallet.deleteBank}${id}`;
    setModalOpen(true);
    setModalContent(
      <DeleteCompo
                ...
                return (
                  <tr
                    ...
                    <td className="px-2">
                      ...
                      <svg
                        onClick={(e) =>
                          handleDelete(e, tbody?.id, tbody?.bankName)
                        }
