...
import CreateMobileBanking from "../../Component/Wallet/CreateMobileBanking";
const tableHead = [
  { id: 1, name: "SL", width: 10 },
  { id: 2, name: "Image", width: 40 },
  { id: 7, name: "Provider Name", width: 150 },
  { id: 3, name: "Number", width: 150 },
  // { id: 5, name: "Status", width: 40 },
  { id: 6, name: "Action", width: 40 },
];

const MobileBanking = ({ isLoading, setIsLoading }) => {
  ...
  const [modalContent, setModalContent] = useState(
    <CreateMobileBanking setModalOpen={setModalOpen} />
  );
  ...
  const getData = async () => {
    ...
    try {
      const response = await axios.get(
        `${process.env.REACT_APP_MAIN_URL}${ENDPOINT.wallet.getMobileBank}${
          WalletID().wallet_id
        }/`,
        {
          headers: {
  ...
  useEffect(() => {
    ...
    function filter() {
      if (searchState !== "") {
        tableDatas.map((dat) => {
          if (
            dat.number.toLowerCase().search(searchState.toLowerCase()) !== -1
          ) {
            arr.push(dat);
          }
          return true;
  ...
  const handleModal = (e) => {
    ...
    setModalOpen(true);
    setModalContent(
      <CreateMobileBanking setModalOpen={setModalOpen} getData={getData} />
    );
  };

  const handleDelete = (e, id, DltTitle) => {
    e.preventDefault();
    const endpoint = `${process.env.REACT_APP_MAIN_URL}${ENDPOINT?.wallet.deleteMobileBank}${id}/`;
    setModalOpen(true);
    setModalContent(
      <DeleteCompo
                ...
                return (
                  <tr
                    ...
                    <td className="px-2">
                      ...
                      <svg
                        onClick={(e) =>
                          handleDelete(e, tbody?.id, tbody?.number)
                        }
